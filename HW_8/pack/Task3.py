...
def directory(dir_name, file_json, file_csv, file_pickle):
    files = []
    directories = []
    for dir_path, dir_name, file_name in os.walk(dir_name):
        ...
        if len(dir_name) != 0:
            for elem in dir_name:
                ...
                directories.append(
                    {'name': elem, 'size': total_size, 'type': 'directory', 'directory': str(dir_path)})
...
if __name__ == '__main__':
    directory(r'D:\Geek Brains\погружение в Python\immersion_in_python\HW_8', 'directories.json', 'directories.csv',
              'directories.pickle')
